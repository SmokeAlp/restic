...
#     if v[0] > v[1]:
#         haventGoodds.append(
#             k)  # добавляю id продукта в список,содежащий id проблемных продуктов, если этого продукта не хватает
#         problems_goods[k] = [
#             v[1]]  # создаю в отдельном словаре {id проблемного продукта: [кол-во этого продукта на складе] }
# print(haventGoodds)  # список idников проблемных продуктов
# print(
#     problems_goods)  # поулчается словарь с ключми из id проблемных продуктов , а значение - это список, в котором под 0-м индексом - это кол-во проблемного продукта на складе
#
...
#         pr_nd_id_for_gd = list(map(lambda x: x[0],
#                                    i.get_needed_products_amount_and_id_for_good_by_good_id()))  # получаю список id нужных продуктов для этого товара
#         print(pr_nd_id_for_gd)
#         probl = list(set(pr_nd_id_for_gd) & set(
#             haventGoodds))  # если есть ли совпадения у списка проблемных idников продуктов и у списка idников продуктов для этого товара
...
#                 problems_goods[k].append(
#                     i.name)  # k - это id проблемного продукта, мы находим такой же id пробелмного продукта в словаре problems_goods, как и k, и добавляем в список этому ключу имя товара i.name и кол-во товара в заказе
...
#     print(
#         f'проблема с прод {k}({v[0]} на складе) для товаров:{v[1::2]}(нужно {v[2::2]} продуктов для {v[2::2]} {v[1::2]}')
#
